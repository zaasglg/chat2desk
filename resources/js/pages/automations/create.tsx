...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Новая автоматизация', href: '/automations/create' },
];
...
export default function AutomationsCreate({ channels, tags }: Props) {
    ...
    const [trigger, setTrigger] = useState('new_chat');
    ...
    const handleSave = () => {
        ...
        const stepsData = steps.map((step, index) => ({
            ...
            position: JSON.stringify({ x: 250, y: index * 150 }),
            next_step_id: steps[index + 1]?.id || null,
        }));

        // Use fetch instead of router.post for complex nested data
        fetch('/automations', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                ...
                trigger,
                trigger_config: trigger === 'keyword' ? { keywords: triggerKeywords } : null,
                is_active: isActive,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Новая автоматизация" />
                        ...
                        <div className="max-w-md mx-auto space-y-4">
                            ...
                            {steps.map((step, index) => {
                                ...
                                return (
                                    <div key={step.id}>
                                        <Card 
                                            className={`cursor-pointer transition-all ${selectedStepId === step.id ? 'ring-2 ring-primary' : ''}`}
                                            onClick={() => setSelectedStepId(step.id)}
                                        >
                                            <div 
                                                className="px-4 py-2 rounded-t-lg flex items-center gap-2 text-white text-sm font-medium"
                                            ...
                                            <CardContent className="p-3">
                                                ...
                                                {step.type === 'send_image' && (
                                                    <p className="text-sm text-muted-foreground">
                                                        {step.config.url ? 'Изображение добавлено' : 'Укажите URL изображения'}
                                                    </p>
                                                )}
                                                {step.type === 'send_video' && (
                                                    <p className="text-sm text-muted-foreground">
                                                        {step.config.url ? 'Видео добавлено' : 'Укажите URL видео'}
                                                    </p>
                                                )}
                                                {step.type === 'add_tag' && (
                                                    <p className="text-sm text-muted-foreground">
